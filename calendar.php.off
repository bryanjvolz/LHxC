...
function check_calendar() {
	...
	if ( $wp_calendar_exists == false && $wp_calendar_config_exists == false ) {
		$new_install = true;
	} elseif ( $wp_calendar_exists == true && $wp_calendar_config_exists == false ) {
		$vone_point_one_upgrade = true;
	} elseif ( $wp_calendar_exists == true && $wp_calendar_config_exists == true && $wp_calendar_config_version_number_exists == false ) {
	...
	if ( $new_install == true ) {
		...
		$wpdb->get_results( $sql );
	} elseif ( $vone_point_one_upgrade == true ) {
		...
		$sql = 'INSERT INTO ' . WP_CALENDAR_CATEGORIES_TABLE . " SET category_id=1, category_name='General', category_colour='#F6F79B'";
		$wpdb->get_results( $sql );
	} elseif ( $vone_point_two_beta_upgrade == true ) {
		$sql = 'INSERT INTO ' . WP_CALENDAR_CONFIG_TABLE . " SET config_item='calendar_version', config_value='1.2'";
		$wpdb->get_results( $sql );
		$sql = 'INSERT INTO ' . WP_CALENDAR_CONFIG_TABLE . " SET config_item='enable_categories', config_value='false'";
		$wpdb->get_results( $sql );
		...
		foreach ( $events as $event ) {
			...
			<tr class="<?php echo $class; ?>">
				...
				if ( $event->event_recur == 'S' ) {
					echo __( 'Never', 'calendar' ); } elseif ( $event->event_recur == 'W' ) {
					echo __( 'Weekly', 'calendar' ); } elseif ( $event->event_recur == 'M' ) {
						echo __( 'Monthly (date)', 'calendar' ); } elseif ( $event->event_recur == 'U' ) {
						echo __( 'Monthly (day)', 'calendar' ); } elseif ( $event->event_recur == 'Y' ) {
							echo __( 'Yearly', 'calendar' ); }
						?>
				</td>
				<td>
				<?php
						// Interpret the DB values into something human readable
				if ( $event->event_recur == 'S' ) {
					echo __( 'N/A', 'calendar' ); } elseif ( $event->event_repeats == 0 ) {
					echo __( 'Forever', 'calendar' ); } elseif ( $event->event_repeats > 0 ) {
						echo stripslashes( $event->event_repeats ) . ' ' . __( 'Times', 'calendar' ); }
					?>
				</td>
				<td>
				<?php
		...
		<div id="linkadvanceddiv" class="postbox">
			<div style="float: left; width: 98%; clear: both;" class="inside">
				...
				<td><input type="text" name="event_title" class="input" size="40" maxlength="30"
					...
					<?php
					if ( ! empty( $data ) ) {
						echo htmlspecialchars( stripslashes( $data->event_title ) );}
					?>
					" /></td>
								</tr>
								<tr>
				<td style="vertical-align:top;"><legend><?php _e( 'Event Description', 'calendar' ); ?></legend></td>
				<td><textarea name="event_desc" class="input" rows="5" cols="50">
				<?php
				if ( ! empty( $data ) ) {
					echo htmlspecialchars( stripslashes( $data->event_desc ) );}
				?>
				</textarea></td>
								</tr>
								<tr>
...
';
				...
				<td><legend><?php _e( 'Event Link (Optional)', 'calendar' ); ?></legend></td>
								<td><input type="text" name="event_link" class="input" size="40" value="
								<?php
								if ( ! empty( $data ) ) {
									echo htmlspecialchars( stripslashes( $data->event_link ) );}
								?>
								" /></td>
								</tr>
								<tr>
				...
				<td>	<input type="text" name="event_time" class="input" size=12
					...
					?>
					" /> <?php _e( 'Optional, set blank if not required.', 'calendar' ); ?> <?php
					_e( 'Current time difference from GMT is ', 'calendar' );
				...
				} elseif ( $data->event_recur == 'W' ) {
					$selected_w = 'selected="selected"';
				} elseif ( $data->event_recur == 'M' ) {
					$selected_m = 'selected="selected"';
				} elseif ( $data->event_recur == 'Y' ) {
					$selected_y = 'selected="selected"';
				} elseif ( $data->event_recur == 'U' ) {
...
function edit_calendar_config() {
	...
	if ( isset( $_POST['permissions'] ) && isset( $_POST['style'] ) ) {
		if ( $_POST['permissions'] == 'subscriber' ) {
			$new_perms = 'read'; } elseif ( $_POST['permissions'] == 'contributor' ) {
			$new_perms = 'edit_posts'; } elseif ( $_POST['permissions'] == 'author' ) {
				$new_perms = 'publish_posts'; } elseif ( $_POST['permissions'] == 'editor' ) {
				$new_perms = 'moderate_comments'; } elseif ( $_POST['permissions'] == 'admin' ) {
					$new_perms = 'manage_options'; } else {
	...
	if ( $allowed_group == 'read' ) {
		$subscriber_selected = 'selected="selected"';} elseif ( $allowed_group == 'edit_posts' ) {
		$contributor_selected = 'selected="selected"';} elseif ( $allowed_group == 'publish_posts' ) {
			$author_selected = 'selected="selected"';} elseif ( $allowed_group == 'moderate_comments' ) {
			$editor_selected = 'selected="selected"';} elseif ( $allowed_group == 'manage_options' ) {
...
<div class="wrap">
	...
	<form name="catform" id="catform" class="wrap" method="post" action="<?php echo bloginfo( 'wpurl' ); ?>/wp-admin/admin.php?page=calendar-categories">
		...
		<?php
	} elseif ( isset( $_POST['mode'] ) && isset( $_POST['category_id'] ) && isset( $_POST['category_name'] ) && isset( $_POST['category_colour'] ) && $_POST['mode'] == 'edit' ) {
		// Proceed with the save
...
function np_of_day( $date ) {
	...
	if ( ( $dom - 7 ) <= 0 ) {
		$instance = 1; } elseif ( ( $dom - 7 ) > 0 && ( $dom - 7 ) <= 7 ) {
		$instance = 2; } elseif ( ( $dom - 7 ) > 7 && ( $dom - 7 ) <= 14 ) {
			$instance = 3; } elseif ( ( $dom - 7 ) > 14 && ( $dom - 7 ) <= 21 ) {
			$instance = 4; } elseif ( ( $dom - 7 ) > 21 && ( $dom - 7 ) < 28 ) {
				$instance = 5; }
			return $instance;
}
...
<span class="event-title-break"></span><br />';
	...
	if ( $event->event_link != '' ) {
		$linky = stripslashes( $event->event_link ); } else {
		$linky = '#'; }
...
function grab_events( $y, $m, $d, $typing ) {
	...
	*/
	...
	$events = $wpdb->get_results(
		'SELECT * FROM ' . WP_CALENDAR_TABLE . " WHERE event_recur = 'Y' AND EXTRACT(YEAR FROM '$date') >= EXTRACT(YEAR FROM event_begin) AND event_repeats = 0 
UNION ALL 
...
ORDER BY event_id"
	...
	*/
	...
	$events = $wpdb->get_results(
		'SELECT * FROM ' . WP_CALENDAR_TABLE . " WHERE event_recur = 'M' AND EXTRACT(YEAR FROM '$date') >= EXTRACT(YEAR FROM event_begin) AND event_repeats = 0 
UNION ALL
...
ORDER BY event_id"
	...
	if ( ! empty( $events ) ) {
		foreach ( $events as $event ) {
			...
			} elseif ( ( $month_begin < $month_end ) && ( strtotime( $event->event_begin ) <= strtotime( $date ) ) ) {
	...
	*/
	...
	$events = $wpdb->get_results(
		'SELECT * FROM ' . WP_CALENDAR_TABLE . " WHERE event_recur = 'U' AND EXTRACT(YEAR FROM '$date') >= EXTRACT(YEAR FROM event_begin) AND event_repeats = 0 
UNION ALL 
...
ORDER BY event_id"
		...
		foreach ( $events as $event ) {
			...
			if ( ( $month_begin == $month_end ) && ( strtotime( $event->event_begin ) <= strtotime( $date ) ) ) {
				if ( np_of_day( $event->event_begin ) == np_of_day( $date ) && $plan[ $day_start_event ] == $plan[ $current_day ] ) {
					if ( $typing == 'calendar' ) {
						$cal_no[ $event->event_id ] = strtotime( $date ); } elseif ( $typing == 'upcoming' ) {
										$tod_no[ $event->event_id ] = strtotime( $date ); } elseif ( $typing == 'todays' ) {
							$tod_no[ $event->event_id ] = strtotime( $date ); }
				}
				if ( $typing == 'calendar' ) {
					$week_no[ $event->event_id ] = $cal_no[ $event->event_id ]; } elseif ( $typing == 'upcoming' ) {
					$week_no[ $event->event_id ] = $tod_no[ $event->event_id ]; } elseif ( $typing == 'todays' ) {
						$week_no[ $event->event_id ] = $tod_no[ $event->event_id ]; }
	...
	*/
	...
	$events = $wpdb->get_results(
		'SELECT * FROM ' . WP_CALENDAR_TABLE . " WHERE event_recur = 'W' AND '$date' >= event_begin AND event_repeats = 0 
UNION ALL
...
ORDER BY event_id"
	...
	if ( ! empty( $events ) ) {
		foreach ( $events as $event ) {
			...
			} elseif ( ( $plan[ $day_start_event ] < $plan[ $day_end_event ] ) || ( $plan[ $day_start_event ] == $plan[ $day_end_event ] ) ) {
				if ( ( $plan[ $day_start_event ] <= $plan[ $current_day ] ) && ( $plan[ $current_day ] <= $plan[ $day_end_event ] ) ) {
					array_push( $arr_events, $event );
				}
			}
...
function calendar() {
	...
	if ( get_option( 'start_of_week' ) == 0 ) {
		$name_days = array(
			1 => __( 'Sunday', 'calendar' ),
			__( 'Monday', 'calendar' ),
			__( 'Tuesday', 'calendar' ),
			__( 'Wednesday', 'calendar' ),
			__( 'Thursday', 'calendar' ),
			__( 'Friday', 'calendar' ),
			__( 'Saturday', 'calendar' ),
		);
	}
	// Choose Monday if anything other than Sunday is set
	else {
		$name_days = array(
			1 => __( 'Monday', 'calendar' ),
			__( 'Tuesday', 'calendar' ),
			__( 'Wednesday', 'calendar' ),
			__( 'Thursday', 'calendar' ),
			__( 'Friday', 'calendar' ),
	...
	$name_months = array(
		1 => __( 'January', 'calendar' ),
		__( 'February', 'calendar' ),
		__( 'March', 'calendar' ),
		__( 'April', 'calendar' ),
		__( 'May', 'calendar' ),
		__( 'June', 'calendar' ),
		__( 'July', 'calendar' ),
		__( 'August', 'calendar' ),
		__( 'September', 'calendar' ),
		__( 'October', 'calendar' ),
		__( 'November', 'calendar' ),
		__( 'December', 'calendar' ),
	);
	...
	if ( $_GET['yr'] <= 3000 && $_GET['yr'] >= 0 && (int) $_GET['yr'] != 0 ) {
		...
		if ( $_GET['month'] == 'jan' || $_GET['month'] == 'feb' || $_GET['month'] == 'mar' || $_GET['month'] == 'apr' || $_GET['month'] == 'may' || $_GET['month'] == 'jun' || $_GET['month'] == 'jul' || $_GET['month'] == 'aug' || $_GET['month'] == 'sept' || $_GET['month'] == 'oct' || $_GET['month'] == 'nov' || $_GET['month'] == 'dec' ) {
			...
			if ( $_GET['month'] == 'jan' ) {
				$t_month = 1; } elseif ( $_GET['month'] == 'feb' ) {
				$t_month = 2; } elseif ( $_GET['month'] == 'mar' ) {
					$t_month = 3; } elseif ( $_GET['month'] == 'apr' ) {
					$t_month = 4; } elseif ( $_GET['month'] == 'may' ) {
							$t_month = 5; } elseif ( $_GET['month'] == 'jun' ) {
							$t_month = 6; } elseif ( $_GET['month'] == 'jul' ) {
								$t_month = 7; } elseif ( $_GET['month'] == 'aug' ) {
								$t_month = 8; } elseif ( $_GET['month'] == 'sept' ) {
									$t_month = 9; } elseif ( $_GET['month'] == 'oct' ) {
									$t_month = 10; } elseif ( $_GET['month'] == 'nov' ) {
											$t_month = 11; } elseif ( $_GET['month'] == 'dec' ) {
										$t_month = 12; }
											$c_month = $t_month;
											$c_day   = date( 'd', ctwo() );
		}
